...
function ucitaj(){
	$.getJSON(putanjaAPI + "/misli", function( jsonData ) {
...
function definirajDogadajeNakonAJAXa(){
	...
	$(".promjena").click(function(){
		...
		$.getJSON(putanjaAPI + "/misao/" + sifra, function( jsonData ) {
...
function postaviNaTablicu(jsonData){
	$("#podaci").html("");
	  $.each( jsonData, function( kljuc, operater ) {
	    ...
	    "<td>" + operater.korisnik + "</td>" +
		"<td>" + operater.misao + "</td>" +
